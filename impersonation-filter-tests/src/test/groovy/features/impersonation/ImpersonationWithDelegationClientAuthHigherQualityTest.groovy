 ...
 */
class ImpersonationWithDelegationClientAuthHigherQualityTest extends ReposeValveTest {
    ...
    def setupSpec() {
        ...
        repose.configurationProvider.applyConfigs("features/impersonation/delegable_clientauthhigher", params)
        repose.start()
    ...
    def "When running with failing admin requests"() {
        ...
        100       | 500               | ""                                             | "GET"         | "/"        | ""          | ['x-auth-token': '123'] | "500"        | "Internal Server Error" | "Failure in Auth-N filter."
        101       | 500               | null                                           | "GET"         | "/"        | ""          | ['x-auth-token': '123'] | "500"        | "Internal Server Error" | "Failure in Auth-N filter."
        102       | 404               | fakeIdentityService.identityFailureXmlTemplate | "GET"         | "/"        | ""          | ['x-auth-token': '123'] | "500"        | "Not Found"             | "Failure in Auth-N filter."
        103       | 401               | fakeIdentityService.identityFailureXmlTemplate | "GET"         | "/"        | ""          | ['x-auth-token': '123'] | "500"        | "Unauthorized"          | "Failure in Auth-N filter."
    }
    ...
    def "When running with failing token requests"() {
        ...
        200       | 500                  | ""                                             | "GET"         | "/"        | ""          | ['x-auth-token': '123'] | "500"        | "Failure in Auth-N filter."                      | 2 //first time needs to admin auth
        201       | 500                  | null                                           | "GET"         | "/"        | ""          | ['x-auth-token': '123'] | "500"        | "Failure in Auth-N filter."                      | 1
        202       | 404                  | fakeIdentityService.identityFailureXmlTemplate | "GET"         | "/"        | ""          | ['x-auth-token': '123'] | "401"        | "Unable to validate token: 123. Invalid token."  | 1
        203       | 401                  | fakeIdentityService.identityFailureXmlTemplate | "GET"         | "/"        | ""          | ['x-auth-token': '123'] | "500"        | "Failure in Auth-N filter."                      | 2 //401 means unauthed
    }
