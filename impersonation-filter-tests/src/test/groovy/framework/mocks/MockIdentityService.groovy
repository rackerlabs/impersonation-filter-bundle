...
/**
 * Created by dimi5963 on 9/26/15.
 */
class MockIdentityService {
    ...
    def impersonated_token = 'this-is-impersonated-token'
    Validator validator;
    ...
    Response handleRequest(Request request) {
        ...
        } else if (isImpersonatePath(nonQueryPath)){
            if (method == "POST") {
                return impersonateTokenHandler(request, xml);
            } else {
                return new Response(405)
            }
        }
    ...
    public static boolean isImpersonatePath(String nonQueryPath) {
        return nonQueryPath.startsWith("/RAX-AUTH/impersonation-tokens")
    }
    ...
    Response impersonateToken(Request request, boolean xml){
        ...
        code = 200;
        template = impersonateTokenJsonTemplate
...
"""
    ...
    def impersonateTokenJsonTemplate =
            """{
    "access": {
        "token": {
            "id": "\${token}",
...
"""
    ...
    def impersonateTokenJsonFailedAuthTemplate =
            """{
    "unauthorized": {
        "code": 401,
